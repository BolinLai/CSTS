...
def pyav_decode_stream(container, start_pts, end_pts, stream, stream_name, buffer_size=0):
    ...
    container.seek(seek_offset, any_frame=False, backward=True, stream=stream)  # move the starting point near the closest key frame to make data loading faster
    frames = {}
    buffer_count = 0
    max_pts = 0
...
def pyav_decode(
    ...
    Args:
        ...
        use_offset (bool): sample a specific clip.
        frames_length_limit (int): only sample in the first N frames (only used for gaze forecasting).
            Set it as -1 if you want to sample in full video segment.
    Returns:
    ...
    if frames_length_limit != -1:  # set the frames_length and duration accordingly
        duration = frames_length_limit / frames_length * duration
        frames_length = frames_length_limit
    ...
    if duration is None:
        ...
        decode_all_video = False
        start_idx, end_idx = get_start_end_idx(  # start and end frame indices
            video_size=frames_length,
            # clip_size=sampling_rate * num_frames / target_fps * fps,  # the original calculation doesn't guarantee uniformly-spaced sampling
            clip_size=((sampling_rate+1) * (num_frames-1) +1) / target_fps * fps,
            clip_idx=clip_idx,
            num_clips=num_clips,
            use_offset=use_offset,
        )
        timebase = duration / frames_length
        video_start_pts = int(start_idx * timebase)  # start and end timestamp
    ...
    return frames, fps, decode_all_video, frames_idx
...
def decode(
    ...
    Args:
        ...
        max_spatial_scale (int): keep the aspect ratio and resize the frame so
            ...
            `torchvision` backend.
        use_offset (bool): sample a specific clip.
        get_frame_idx (bool): return indices of all frames as well.
        frames_length_limit (int): only sample in the first N frames (only used for gaze forecasting).
              Set it as -1 if you want to sample in full video segment.
    Returns:
        frames (tensor): decoded frames from the video.
    ...
    try:
        if backend == "pyav":
            frames, fps, decode_all_video, frames_idx = pyav_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
                ...
                target_fps,
                use_offset=use_offset,
                frames_length_limit=frames_length_limit,
            )
    ...
    # clip_sz = sampling_rate * num_frames / target_fps * fps  # the original calculation doesn't guarantee uniformly-spaced sampling
    clip_sz = ((sampling_rate+1) * (num_frames-1) + 1) / target_fps * fps
    start_idx, end_idx = get_start_end_idx(
        frames.shape[0],
        clip_sz,
    ...
    if get_frame_idx:
        # gaze estimation
        frames_idx = temporal_sampling(frames_idx, start_idx, end_idx, num_frames)
