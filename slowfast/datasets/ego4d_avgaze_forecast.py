...
class Ego4d_av_gaze_forecast(torch.utils.data.Dataset):
    """
    Ego4d video loader. Construct the Ego4d video loader, then sample clips from the videos.
    """

    def __init__(self, cfg, mode, num_retries=10):
        """
        Construct the Ego4d video loader with a given csv file. The format of the csv file is:
        ```
        path_to_video_1
        path_to_video_2
        ...
    ...
    def __getitem__(self, index):
        ...
        sampling_rate = utils.get_random_sampling_rate(self.cfg.MULTIGRID.LONG_CYCLE_SAMPLING_RATE, self.cfg.DATA.SAMPLING_RATE)  # = 8
        ...
        for i_try in range(self._num_retries):
            ...
            frame_length = 86  # only sample in the first 90 frames for gaze forecasting
            ...
            if self.mode == 'train':
                ...
                labels_idx = np.arange(last_idx+1, last_idx+1+ori_frame_length-frame_length)
            else:
                labels_idx = np.arange(frame_length, ori_frame_length)  # array([86, 87, ..., 148, 149])
